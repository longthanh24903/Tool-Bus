...
const SEO_TOOLS: {
  ...
  {
    type: "title",
    label: "Tạo Tiêu đề (Titles)",
    icon: <DocumentTextIcon className="w-6 h-6" />,
  },
  {
    type: "description",
    label: "Tạo Mô tả (Description)",
...
export const ExportAndSeoTools: React.FC<ExportAndSeoToolsProps> = ({
  fullStoryText,
  topic,
  language,
}) => {
  ...
  const handleGenerateSeo = useCallback(
    async (type: SeoContentType) => {
      setIsLoading(type);
      setError(null);
      try {
        const results = await generateSeoContent(
          fullStoryText,
          topic,
          language,
          type
        ...
        setSeoResults((prev) => {
          ...
          if (existing) {
            return prev.map((r) =>
              r.type === type ? { ...r, content: results } : r
            );
          }
  ...
  const handleExport = () => {
    const blob = new Blob([fullStoryText], {
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <h4 className="flex items-center text-base font-semibold text-indigo-500">
                  {tool.icon}
              ...
              {result && (
                <ul className="space-y-2 max-h-[500px] overflow-y-auto">
                  {result.content.map((item, index) => (
                    ...
                    >
                      <span className="text-gray-900 flex-1 pr-2 whitespace-pre-wrap">
                        {item}
                      </span>
                      <button
                        onClick={() =>
                          handleCopy(item, `${tool.type}-${index}`)
                        }
                        className="text-gray-400 hover:text-white transition-colors flex-shrink-0"
                        title="Sao chép"
                      >
                        {copiedIndex === `${tool.type}-${index}` ? (
